...
ProblemData<dim>::ProblemData(const std::string &param_file) {
    params.declare_entry("kappa","1",Patterns::Double(),"Robin boundary condition parameter");
    params.declare_entry("c","1",Patterns::Double(),"Another parameter");
    params.declare_entry("geometry","[0,1]x[0,1]",Patterns::Anything());
    params.declare_entry("rhs","8 - 4 * (x*x+y*y)",Patterns::Anything());
    params.declare_entry("solution","(x*x + y*y) * (x*x + y*y) / 4. - 2. * (x*x + y*y)",Patterns::Anything());
    params.parse_input(param_file);
    rhs.initialize(FunctionParser<dim>::default_variable_names(),params.get("rhs"),typename FunctionParser<dim>::ConstMap());
    solution.initialize(FunctionParser<dim>::default_variable_names(),params.get("solution"),typename FunctionParser<dim>::ConstMap());
}
        ...
        for (unsigned int q_index = 0; q_index < n_q_points; ++q_index) {
            for (unsigned int i = 0; i < dofs_per_cell; ++i) {
                ...
                cell_rhs(i) += (fe_values.shape_value(i, q_index) *
                                sol_base.rhs.value(fe_values.quadrature_point(q_index)) *
                                fe_values.JxW(q_index));
            }
        }
                    ...
                    for (unsigned int i = 0; i < dofs_per_cell; ++i) {
                        for (unsigned int j = 0; j < dofs_per_cell; ++j) {
                            cell_matrix(i, j) += (fe_face_values.shape_value(i, q_index) *
                                                  ...
                                                  sol_base.params.get_double("kappa") * fe_face_values.JxW(q_index));
                        }
                        cell_rhs(i) += fe_face_values.shape_value(i, q_index) * sol_base.params.get_double("kappa") * sol_base.params.get_double("c") * fe_face_values.JxW(q_index);
                    }
                }
            }
        }
