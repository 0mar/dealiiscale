...
template<int dim>
void
MicroSolver<dim>::integrate_cell(const typename DoFHandler<dim>::active_cell_iterator &cell, FEValues<dim> &fe_values,
...
void MicroSolver<dim>::assemble_system() {
    ...
    for (const auto &cell: dof_handler.active_cell_iterators()) {
        integrate_cell(cell, fe_values, quadrature_formula.size());
    }
    for (unsigned int k = 0; k < num_grids; k++) {
